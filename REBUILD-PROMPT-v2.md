...
- Visual identity: cream/brown palette (`#f9f6ec` bg, `#3d2817` dark brown, `#e0ddc8`/`#d0cdb8`
  borders, white bands), system font stack, generous spacing, rounded cards, dark CTA band. Pull
  exact tokens from the current CSS; centralize as CSS custom properties.
...
- Content collections (Markdown) for `nobody-built-the-first-mile`, glossary, privacy; `.astro`
  pages for home, about, market map, discovery.
...
| `/discovery` | `discovery.html` | Tally embed, `noindex,nofollow`, out of sitemap, disallowed in robots.txt |
...
`person_profiles: 'identified_only'`, register `{ site: 'eagleridge.io' }`. Keep
`discovery_page_viewed` on `/discovery`.
...
- `sitemap.xml` (`@astrojs/sitemap`, exclude `/discovery`) + human-readable `sitemap.md`.
...
2. All seven pages at parity (oracle-verified), same look and message.
